...
class VOSDataset(Dataset):
    ...
    """
    def __init__(self, im_root, ir_root, gt_root, max_jump, is_bl, subset=None, num_frames=3, max_num_obj=3, finetune=False):
    ...
    def __getitem__(self, idx):
        ...
        vid_ir_path = path.join(self.ir_root, video)
        ...
        while trials < 5:
            ...
            for f_idx in frames_idx:
                ...
                reseed(sequence_seed)
                this_ir = Image.open(path.join(vid_ir_path, jpg_name)).convert('RGB')
                this_ir = self.all_im_dual_transform(this_ir)
                this_ir = self.all_im_lone_transform(this_ir)
                reseed(sequence_seed)
                ...
                reseed(pairwise_seed)
                this_ir = self.pair_im_dual_transform(this_ir)
                this_ir = self.pair_im_lone_transform(this_ir)
                reseed(pairwise_seed)
                this_gt = self.pair_gt_dual_transform(this_gt)

                this_im = self.final_im_transform(this_im)
                this_ir = self.final_im_transform(this_ir)
                this_gt = np.array(this_gt)

                images.append(this_im)
                infara.append(this_ir)
                masks.append(this_gt)

            images = torch.stack(images, 0)
            infara = torch.stack(infara, 0)
        ...
        cls_gt = np.zeros((self.num_frames, 384, 384), dtype=int)
        first_frame_gt = np.zeros((1, self.max_num_obj, 384, 384), dtype=int)
