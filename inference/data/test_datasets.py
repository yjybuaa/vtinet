...
class RGBTTestDataset:
    def __init__(self, root):
        self.root = root
        
        self.mask_dir = path.join(root, 'Annotations')
        self.image_dir = path.join(root, 'RGBImages')
        self.ir_dir = path.join(root, 'ThermalImages')

        video_list = glob.glob(f'{root}/RGBImages/*')
        video_list = [v.split('/')[-1] for v in video_list]
        video_list.sort()

        self.vid_list = video_list

    def get_datasets(self):
        for video in self.vid_list:
            yield VideoReader(video, 
                path.join(self.image_dir, video), 
                path.join(self.ir_dir, video), 
                path.join(self.mask_dir, video)
            )
